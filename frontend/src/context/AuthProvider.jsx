...
export const AuthProvider = ({ children }) => {
    ...
    React.useEffect(()=> {
        let requestUserSchool= async () => {
            ...
            return fetch(`http://127.0.0.1:8000/api/schools/${userInfo?.school_id}`, {
                method: 'GET',
                headers: new Headers({
                    Authorization: `Bearer ${authToken}`,
                }),
            })
            .then((response) => response.json()).then((json) => setUserSchool(json));;
            }
        }
        requestUserSchool()
    }, [userInfo])
    ...
    // React.useEffect(() => {
    //     let requestUsers = async () => {
    //       return fetch(
    //         `http://127.0.0.1:8000/api/schools/${userInfo.school_id}/users`,
    //         {
    //           method: "GET",
    //           headers: new Headers({
    //             Authorization: `Bearer ${authToken}`,
